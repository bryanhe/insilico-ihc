...
def _get_downsample(slide, magnification):
    ...
    if ds not in [1, 2]:
        raise ValueError("Need downsample of {}.".format(ds))
    return ds
...
def read_region_at_mag(slide, location, magnification, size, downsample=1):
    ...
    # If the read runs off of the image, the RGBA is (0, 0, 0, 0) (completely transparent black pixel).
    # The alpha_composite is to make the missing part white instead of black.
    X = PIL.Image.alpha_composite(PIL.Image.new("RGBA", X.size, (255, 255, 255)), X)
    X = X.convert("RGB")
